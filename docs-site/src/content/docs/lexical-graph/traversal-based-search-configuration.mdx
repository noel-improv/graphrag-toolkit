...
### Topics
  ...
  - [Token budget](#token-budget)
    ...
    - [token_truncation_mode](#token_truncation_mode)
...
The `max_search_results`, `max_statements_per_topic` and `max_statements` parameters bound the result size by *counts*. As an alternative, you can bound the context by a *token budget* - useful when you want to fill a fixed share of the generator's context window rather than reason about statement counts.

#####  `max_context_tokens`

`max_context_tokens` (default `None`) sets a token budget for the statements passed to the response generator, enforced by the `TruncateByTokens` processor. When unset, the processor is a no-op and the count-based parameters apply as usual.

#####  `token_truncation_mode`

`token_truncation_mode` selects how statements are chosen to fill the budget:

  - `global_rank` (default): flattens all statements across topics and sources, sorts them by relevance score, and fills the budget from the top. This keeps the most relevant statements regardless of which topic or source they belong to, and is the recommended general-purpose mode.
  - `per_topic_cap`: gives each topic an equal share of the budget (`max_context_tokens / number_of_topics`). This is the token-based analog of `max_statements_per_topic` - use it when you want to preserve breadth across topics rather than relevance-first selection.
...
```python
    query_engine = LexicalGraphQueryEngine.for_traversal_based_search(
        ...
        token_truncation_mode='global_rank'
    )
```

:::note
Token counts are computed with tiktoken's `cl100k_base` encoding. This is a consistent approximation for relative budgeting; it is not an exact token count for non-OpenAI generators such as Anthropic Claude. To avoid an empty context, the first statement is always kept even if it alone exceeds the budget.
:::
